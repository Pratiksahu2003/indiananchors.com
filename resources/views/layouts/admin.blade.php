    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: true }">
        ...
        <aside class="bg-slate-900 text-slate-300 w-64 flex-shrink-0 flex flex-col transition-all duration-300" 
            ...
            <div class="p-6 flex items-center justify-between">
                <span class="text-xl font-bold text-white flex items-center gap-2">
                    <i class="fas fa-newspaper text-indigo-400"></i> PostApp
                </span>
            ...
            <nav class="flex-1 px-4 space-y-1 py-4 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800 {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-600 text-white' : '' }}">
                    <i class="fas fa-chart-line w-5"></i> Dashboard
                </a>
                <div class="pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase px-4">Content Management</div>
                <a href="{{ route('admin.posts.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800 {{ request()->routeIs('admin.posts.*') ? 'bg-indigo-600 text-white' : '' }}">
                    <i class="fas fa-file-alt w-5"></i> Blog Posts
                </a>
                <a href="{{ route('admin.categories.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800 {{ request()->routeIs('admin.categories.*') ? 'bg-indigo-600 text-white' : '' }}">
                    <i class="fas fa-folder w-5"></i> Categories
                </a>
                <a href="{{ route('admin.tags.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800 {{ request()->routeIs('admin.tags.*') ? 'bg-indigo-600 text-white' : '' }}">
            ...
            <div class="p-4 border-t border-slate-800">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="flex items-center gap-3 px-4 py-2 w-full text-left rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-800">
        ...
        <main class="flex-1 flex flex-col h-full bg-[#f1f5f9] overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-8">
                @if(session('success'))
                    <div class="bg-emerald-50 text-emerald-700 p-4 rounded-xl border border-emerald-200 mb-6 flex items-center gap-3">

...
<article class="bg-white min-h-screen relative overflow-hidden font-dm">
    ...
    <div
        class="absolute inset-x-0 top-1/4 -translate-y-1/2 flex justify-center pointer-events-none select-none overflow-hidden z-0 opacity-40">
        <span
            class="text-[25vw] font-syne font-black text-slate-50 uppercase leading-none transform -rotate-6">STORY</span>
    </div>
    ...
    <header class="relative min-h-[50vh] flex items-center justify-center pt-32 pb-20 overflow-hidden bg-slate-950">
        ...
        <img src="{{ Storage::url($post->featured_image) }}" fetchpriority="high" alt="{{ $post->title }}"
            class="absolute inset-0 w-full h-full object-cover opacity-20 mix-blend-luminosity scale-110 blur-[1px]">
        @else
        <img src="{{ asset('img/blog.avif') }}" fetchpriority="high"
            class="absolute inset-0 w-full h-full object-cover opacity-10 mix-blend-luminosity">
        @endif

        <!-- Background Accents -->
        <div class="absolute top-0 right-0 w-[600px] h-[600px] bg-[#c9a227]/5 rounded-full blur-[120px] animate-pulse">
        </div>

        <div class="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 relative z-20">
            <div class="max-w-4xl mx-auto text-center" data-aos="fade-up">
                <nav
                    class="flex items-center justify-center gap-3 mb-8 text-[9px] font-black text-[#c9a227] uppercase tracking-[0.3em]">
                ...
                <h1
                    class="text-3xl md:text-5xl lg:text-7xl font-syne font-black text-white mb-10 tracking-tighter leading-none uppercase drop-shadow-2xl">
                    {{ $post->title }}
                </h1>
                ...
                <div class="flex items-center justify-center gap-3">
                    <div class="w-10 h-10 rounded-full border border-[#c9a227]/30 p-0.5">
                        <div
                            class="w-full h-full rounded-full bg-slate-800 flex items-center justify-center text-[8px] font-black text-white tracking-widest uppercase">
                            IA</div>
                    </div>
                    <div class="text-left leading-none">
                        <p class="text-white font-black uppercase tracking-widest text-[9px] mb-1">Team Indian Anchors
                        </p>
    ...
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 py-12 md:py-20 lg:py-24 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-14 lg:gap-20 items-start">
            ...
            <aside class="md:col-span-4 space-y-6 md:sticky md:top-32 order-2 md:order-1" data-aos="fade-right">
                ...
                <div
                    class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden group">
                    <h4
                        class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-950 mb-8 flex items-center gap-3 relative z-10">
                    ...
                    <div class="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-3">
                        ...
                        <a href="{{ route('blog.category', $cat->slug) }}"
                            class="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-50 hover:border-[#c9a227] hover:bg-white transition-all group">
                            <span
                                class="text-[10px] font-black uppercase tracking-widest text-slate-950">{{ $cat->name }}</span>
                            <span
                                class="text-[8px] font-dm text-slate-400 group-hover:text-[#c9a227]">{{ $cat->posts_count ?? '' }}</span>
                        </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden">
                    <h4
                        class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-950 mb-6 flex items-center gap-3">
                        <i class="bi bi-info-circle-fill text-[#c9a227]"></i> Story Info
                    </h4>
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-2 border-b border-slate-50">
                            <span class="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Industry
                                focus</span>
                            <span
                                class="text-[9px] font-black uppercase text-slate-950 tracking-widest">Entertainment</span>
                        </div>
                        <div class="flex items-center justify-between py-2 border-b border-slate-50">
                            <span class="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Mastery
                                Level</span>
                            <span class="text-[9px] font-black uppercase text-slate-950 tracking-widest">Expert</span>
                        </div>
                        <div class="flex items-center justify-between py-2">
                            <span class="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Reading
                                Time</span>
                ...
                <div class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-2">
                            <span
                                class="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Visibility</span>
                            <span
                                class="px-3 py-1 bg-green-50 text-green-600 rounded-full text-[8px] font-black uppercase tracking-widest">Published</span>
                        </div>
                        <div class="flex items-center justify-between py-2">
                            <span class="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Expert
                                Insight</span>
                            <span
                                class="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[8px] font-black uppercase tracking-widest">Verified</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden group">
                    <h3
                        class="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-slate-400 mb-8 flex items-center gap-3 relative z-10">
                    ...
                    <div class="flex flex-wrap gap-2 max-h-[350px] overflow-y-auto custom-scrollbar pr-3">
                        ...
                        <a href="{{ route('blog.tag', $tag->slug) }}"
                            class="px-4 py-2 rounded-xl bg-slate-50 border border-slate-100 text-[9px] font-black uppercase tracking-widest text-slate-500 hover:bg-[#c9a227] hover:text-slate-950 hover:border-[#c9a227] transition-all">
                            #{{ $tag->name }}
                        </a>
                        @endforeach
                    </div>
                        ...
                        <a href="{{ route('blog.show', $rp->slug) }}" class="group/post">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="w-20 h-20 rounded-2xl overflow-hidden shrink-0 border border-slate-100">
                                    <img src="{{ Storage::url($rp->featured_image) }}"
                                        class="w-full h-full object-cover group-hover/post:scale-110 transition-transform duration-700">
                                </div>
                                @endif
                                <div>
                                    <h4
                                        class="text-[11px] font-syne font-black text-slate-950 group-hover/post:text-[#c9a227] leading-tight transition-all uppercase tracking-tighter line-clamp-2">
                                        {{ $rp->title }}
                                    </h4>
                                    <p class="text-[8px] font-black uppercase tracking-widest text-slate-400 mt-2">
                                        {{ $rp->published_at->format('M Y') }}</p>
                                </div>
                            </div>
                        </a>
                        @empty
            ...
            <main class="md:col-span-8 space-y-12 lg:space-y-16 order-1 md:order-2" data-aos="fade-up"
                ...
                <div class="pt-12 border-t border-slate-100">
                    <div class="flex flex-wrap items-center gap-3">
                        <span class="text-[9px] font-black uppercase text-slate-400 tracking-widest mr-4">Story
                            Themes:</span>
                        @foreach($post->tags as $tag)
                        <a href="{{ route('blog.tag', $tag->slug) }}"
                            class="px-5 py-2 rounded-full bg-slate-50 border border-slate-100 text-[9px] font-black uppercase tracking-widest text-slate-500 hover:bg-[#c9a227] hover:text-slate-950 hover:border-[#c9a227] transition-all">
                            #{{ $tag->name }}
                        </a>
                        @endforeach
                    </div>
